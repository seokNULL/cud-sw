...
// Constant rows — pre-initialized once by the caller; never bump-allocated.
static constexpr uint32_t kZeroRow = 101u;  // all bits = 0  (AND bias / OR frac init)
static constexpr uint32_t kOnesRow = 102u;  // all bits = 1  (OR bias)

// Mode-0 MAJ3 compute group base for instruction generators.
...
// General scratch rows start here (after the compute group's last row = 121).
// Available range: [122, 901)  →  779 rows for temporaries.
